...
**Proven:** bounded Hermes + FreeLLMAPI carrier, 90% qualification, live Modal deployment, real browser OAuth, stable browser WebSocket/chat after the compression fix, and production deployment of the hard web-only TUI operator pin.

**Implemented but not yet restart-proven:** persistence of interactive dashboard state across an intentional restart/scale-down.

**Still externally gated:** fresh exact-candidate review required by the relevant governance before acceptance/merge.
